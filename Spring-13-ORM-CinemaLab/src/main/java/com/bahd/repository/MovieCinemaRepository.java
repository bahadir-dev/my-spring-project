...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema,Long> {
    ...
    //-------JPQL QUERIES --------///
    //write a jpql query to list all movie cinema swith higher than a specific date
    ...
    //--------- Native QUERIES ---------//
    //write a native query to count all movie cinema by cinema id
    @Query(value = "SELECT COUNT(*) FROM movie_cinema WHERE cinema_id = ?1",nativeQuery = true)
    Integer countByCinemaId(Long cinemaId);

    //write a native query that returns all movie cineams by location name
    @Query(value = "SELECT * FROM movie_cineam mc JOIN cinema c " +
